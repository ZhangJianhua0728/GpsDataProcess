...
class DimensionReduce:
    """
    @description  : 构建一个用于数据降维的类
    ---------
    """
    def __init__(self,data: pd.DataFrame, n_pc=None):
        """
        @description  : 数据降维类的初始化函数
        ---------
        @data  : 需要降维的数据 DataFrame
        -------
        """
        self.data = data
        self._PCA(n_pc)
    ...
    def _PCA(self, n_pc):
        """
        @description  : 创建主成分分析对象，并对标准化后的数据进行主成分分析
        -------
        """
        if n_pc is None:
            self.pca = PCA()
    ...
    def calc_total_variance_explain(self) -> pd.DataFrame:
        """
        @description  : 计算每个主成分的特征值、方差贡献率和累计贡献率
        ---------
        @Returns  : 返回一个总方差解释表 DataFrame
        -------
        """
        # 获取特征值和方差贡献率
    ...
    def calc_PC_loadings(self) -> pd.DataFrame:
        """
        @description  : 计算各指标数据的主成分因子载荷
        ---------
        @Returns  : 返回主成分因子载荷 DataFrame
        -------
        """
    ...
    def calc_RC_loadings(self) -> pd.DataFrame:
        ...
        -------
        """
        fa = FactorAnalysis(n_components=self.data.shape[1], rotation='varimax')
        fa.fit(self.data)
...
def show_scree_plot(pca_df:pd.DataFrame):
    """
    @description  : 绘制主成分与解释方差的碎石图
    ---------
    """
    fig, ax = plt.subplots(figsize=(4,3))
    # 绘制碎石图
    plt.plot(range(1, len(pca_df) + 1), pca_df['Variance'], '-o', linewidth=1, color='#1a2933')
...
def show_correlation_plot(loading_df: pd.DataFrame, x_cols:list, text_labels: list, xy_labels:list):
    ...
    Args:
        loading_df (pd.DataFrame): _description_
        x_cols (list): _description_
        text_labels (list): _description_
        xy_labels (list): _description_
    """
    # 创建坐标轴
    fig,ax = plt.subplots(figsize=(4,3))
    # 计算每个向量的长度
    ...
    ax.quiver([0]*len(loading_df), [0]*len(loading_df), loading_df[x_cols[0]], loading_df[x_cols[1]], linewidth=0.8, 
    ...
    for i in range(len(loading_df)):
        ...
        ax.text(loading_df[x_cols[0]][i], loading_df[x_cols[1]][i],text_labels[i], fontsize=8, 
