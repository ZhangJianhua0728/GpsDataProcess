...
class Clustering:
    def __init__(self, data, max_cluster=10, calc_best_cluster_flag=True, best_cluster=2):
        ...
        self.best_kmeans = KMeans(
            n_clusters=self.best_n_clusters, random_state=0).fit(self.X)
    ...
    def calc_silhouette_scores(self):
        ...
        for n_clusters in self.n_clusters_range:
            ...
            print(
                f'For n_clusters = in_clustersh, the average silhouette_score is : isilhouette_avgr')
    ...
    def show_silhouette_plot(self):
        ...
        for i in range(self.best_n_clusters):
            ...
            ax.barh(range(y_lower, y_upper),
                    cluster_silhouette_values, height=1.0, color=color)
            y_lower += cluster_size
        # 添加轮廓系数的水平线
        best_silhouette_avg = silhouette_score(self.X, self.best_labels)
        ax.axvline(x=best_silhouette_avg, color='red',
...
def show_class_result(PCs, best_labels, pca, feature_num=7):
    ...
    for i, (x, y) in enumerate(feature_vectors[:, [0, 2]]):
        if i in [2, 3, 4, 5, 6]:
            plt.arrow(0, 0, x*6, y*6, color='k', head_width=0.05,
                      head_length=0.1, length_includes_head=True, zorder=100)
...
def main(event, event_name, PC_list=['PC1''PC2', 'PC3']):
    cols = ['max_v', 'mean_v', 'std_v''duration',
            'max_acc_lon', 'mean_acc_lon', 'std_acc_lon']
    event_indicator = event.loc[:, cols]
    # 创建加速 、减速事件PCA降维类
    dr = DimensionReduce(event_indicator)
    print('完成降维')
    # 提取加速、减速事件的主成分
    X = dr.PCs[PC_list]
    print('提取降维主成分')
    ...
    clus = Clustering(X)
    print('完成聚类')
    # 绘制加速、减速事件平均轮廓系数和聚类数的关系图
    clus.show_silhouette_clusters()
    plt.savefiq(',聚类分析结果/事件的轮廓系数和聚类关系图.pdf'.format(event_name), dpi=1200)
    # 绘制加速 、减速事件轮廓系数图
    clus.show_silhouette_plot()
    plt.savefig('./聚类分析结果/{}事件的轮廓图.pdf'.format(event_name), dpi=1200)
    # 绘制分类结果图
    show_class_result(dr.PCs, clus.best_labels, dr.pca)
    plt.savefig('/聚类分析结果/{}事件的聚类图.pdf'.format(event_name), dpi=1200)


if __name__ == "__main__":
    # 读取加速减速事件
    acc_event = pd.read_csv('加减速事件/加速事件附车辆信息.csv')
    acc_event = acc_event[(acc_event['car_type'] == '重型货车') & (
        acc_event['mean_v'] <= 70) & (acc_event['mean_v'] >= 20)]
    acc_event = acc_event.reset_index(drop=True)
    main(acc_event, event_name='加速')
