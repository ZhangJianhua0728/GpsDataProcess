...
## 获取最优旋转角度
def get_opt_theta(data,cx=C_a[0]，cy=C_j[0]，rx=radius_a[0],ry=radius_j[0]):theta = np.array([i for i in range(1，180，1)])
# 计算角度phi,使更多数据小于PPL_1
rot_angle_inn_point = []
for i in range(len(theta)):
inner_point = data.apply(lambda df: 1 if calc_rot_loc(df['acc_lon'],df['jerk'],theta[i],cx, cy,rx, ry) <= 1 else 0, axis=1)rot_angle_inn_point.append([theta[i]，inner_point.sum()])print(":[},}]".format(i, theta[i], inner_point.sum()))max_val = max(rot_angle_inn_point，key=lambda x: x[1])opt_theta = max_val[o]print('最佳旋转角度为'.format(opt_theta))
return opt_theta
