...
def main(data):
    ...
    opt_theta = 139
    # 计算每个驾驶员的DTV_V， DTV-V-ratio
    DTV-V, DTV-v-ratio = calc-DTV-v(data, thre_V=80）
    ...
    ＃ 计算每个驾驶员的总行驶里程
    total_mileage = data.groupby("trip_num"JI"total_mizeage" J.meanC).sum C)
    # 记录每个驾驶员的DTV_PPAL对应的指标，以及驾驶员工D和总的行驶里程
    DTV_PPAL_res = []
    DTV_PPAL_res.append([data[ phone '][e],DTV_V, DTV_v_ratio, DTV_acc_lat, DTV_acc_lat_ratio, PPAL1, PPAL2, PPAL, total mileage])
    columns = ['phone', 'DTV_v', 'DTV _v_ratio', 'DTV_acc_lat', 'DTV_acc_lat_ratio', 'PPALI',
    "PPAL2', 'PPALs',
    'total_mileage"]
    DTV_PPAL_res_df = pd. DataFrame (DTV_PPAL_res, columns=columns)
    DTV_PPAL_res_df['DTR_1g'] = DTV_PPAL_res_df.apply(Lambda df: calc_DTR_Lg([1,2,3,5],df['DTV_v'],df['PPAL1'], df [ 'PPAL2" ], df [ 'PPAL3' ], df[ 'total_mileage ' ]) , axis=1)
    ＃ 显示PPAL图
    show_PPAL (data, opt_theta)
    print(DTV_PPAL-res_0+)

if __name__ == '__main__':
    data = pd.read_CSV('/Users/zhangjianhua/Desktop/明觉:公路院项目/A_ Code_MingJueProject/qps款据处理/14462543665_行程指标.CSV')
    main(data)