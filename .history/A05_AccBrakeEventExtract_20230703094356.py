...
class EventExtract:
    ...
    def _basic_indicators(self, grouped, result):
        ...
        result['duration'] = grouped['time'].apply(
            lambda x: x.max() - x.min()).dt.total_seconds()+1
...
if __name__ == "__main__":
    filename = '样例数据分析/14462543665_行程指标.csv'
    data = pd.read_csv(filename)
    EveExt = EventExtract(data)
    acc_event = EveExt.acc_lon_event()
    dec_event = EveExt.dec_lon_event()
    acc_event.to_csv('./样例数据分析/14462543665_纵向加速事件.csv', index=False)
    dec_event.to_csv('./样例数据分析/14462543665_纵向减速事件.csv', index=False)
