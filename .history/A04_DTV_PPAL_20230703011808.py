...
def calc_PPAL(data: pd.DataFrame, opt_theta):
    data['PPAL'] = data.apply(lambda df: cls_PPAL(
        df['acc_lon'], df['jerk'], opt_theta, C_a, C_j, radius_a, radius_j), axis=1)
...
def calc_DTR_lg(alpha, DTV_v, PPAL1, PPAL2, PPAL3, total_mileage):
    ...
    """
    DTR_lg = (alpha[0]*DTV_v+alpha[1]*PPAL1+alpha[2]
              * PPAL2+alpha[3]*PPAL3) / total_mileage
    return DTR_lg
...
def show_PPAL(data, opt_theta):
    ...
    xx1, yy1 = ellipse(C_a[0], C_j[0], radius_a[0],
                       radius_j[0], angle=opt_theta)
    xx2, yy2 = ellipse(C_a[1], C_j[1], radius_a[1],
                       radius_j[1], angle=opt_theta)
    xx3, yy3 = ellipse(C_a[2], C_j[2], radius_a[2],
                       radius_j[2], angle=opt_theta)
    ...
    # 提取绘图数据
    show_data = pd.concat([data[data["PPAL"] == 0].sample(n=70), data[data["PPAL"] == 1].sample(
        n=20), data[data["PPAL"] == 2].sample(n=10), data[data["PPAL"] == 3].sample(n=2)])
    # 设置不同类别的颜色和形状
    colors = ['#121a2a', '#224b8f', '#b7ba6b', '#f15b6c']
    markers = ['o', 's', '^', '*']
    # 绘制散点图
    for i, group in show_data.groupby('PPAL'):
        plt.scatter(group['acc_lon'], group['jerk'],
...
def main(data):
    ...
    # 计算每个驾驶员的总行驶里程
    total_mileage = data.groupby("trip_num")["total_mizeage"].mean().sum()
    # 记录每个驾驶员的DTV_PPAL对应的指标,以及驾驶员工D和总的行驶里程
    DTV_PPAL_res = []
    DTV_PPAL_res.append([data['phone'][0], DTV_v, DTV_v_ratio, DTV_acc_lat,
                        DTV_acc_lat_ratio, PPAL1, PPAL2, PPAL3, total_mileage])
    columns = ['phone', 'DTV_v', 'DTV _v_ratio', 'DTV_acc_lat',
               'DTV_acc_lat_ratio', 'PPAL1', 'PPAL2', 'PPAL3', 'total_mileage']
    DTV_PPAL_res_df = pd.DataFrame(DTV_PPAL_res, columns=columns)
    DTV_PPAL_res_df['DTR_lg'] = DTV_PPAL_res_df.apply(lambda df: calc_DTR_lg(
        [1, 2, 3, 5], df['DTV_v'], df['PPAL1'], df['PPAL2'], df['PPAL3'], df['total_mileage']), axis=1)
    # 显示PPAL图
...
if __name__ == '__main__':
    data = pd.read_CSV(
        '/Users/zhangjianhua/Desktop/明觉:公路院项目/A_ Code_MingJueProject/qps款据处理/14462543665_行程指标.CSV')
    main(data)
